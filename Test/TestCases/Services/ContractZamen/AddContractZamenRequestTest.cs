...
public class AddContractZamenRequestTest
{
    ...
    public async Task AddContractZamenRequest_Fail1()
    {
        ...
        var handler = new AddContractZamenRequestHandler(moq.Context.Object, mockMediator.Object);

        var request = new AddContractZamenRequest();
    ...
    [Fact]
    public async Task AddContractZamenRequest_Fail2()
    {
        mockMediator.Setup(x => x.Send(It.IsAny<InquiryCustomerV2Request>(), CancellationToken.None)).ReturnsAsync(
            new ApiResult<long>(200, true) { Data = 1 });

        moq.Context.Setup(x => x.ContractZamens.Add(It.IsAny<Core.Entities.ContractZamen>()));
    ...
    [Fact]
    public async Task AddContractZamenRequest_Success()
    {
        mockMediator.Setup(x => x.Send(It.IsAny<InquiryCustomerV2Request>(), CancellationToken.None)).ReturnsAsync(
            new ApiResult<long>(200, true) { Data = 1 });
